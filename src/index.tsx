...
app.post('/api/hasie/import', async (c) => {
  try {
    ...
    if (messageText.trim() === '[끝]') {
      ...
      // OUT Rank 처리: 글로벌 방식 - 이번 세션에 없는 모든 제품을 OUT으로
      // 이번 세션에 업데이트된 제품 목록 가져오기 (정상 순위만)
      const { results: sessionProducts } = await DB.prepare(`
        SELECT DISTINCT product_link
        FROM hasie_rankings
        WHERE update_session_id = ? AND out_rank = 0
      `).bind(sessionId).all();
      ...
      if (sessionProductLinks.length > 0) {
        ...
        // 이번 세션에 없는 모든 제품을 OUT으로 이동
        // INSERT OR IGNORE + UNIQUE 제약조건으로 중복 방지
        await DB.prepare(`
        ...
        `).bind(msgDate, sessionId, ...sessionProductLinks).run();
      }
      ...
      if (sessionProductLinks.length > 0) {
        ...
        await DB.prepare(`
          ...
          WHERE h1.out_rank = 0
            AND h1.rank != 201
            AND h1.product_link NOT IN (${placeholders})
        `).bind(messageDate, sessionId, ...sessionProductLinks).run();
      }
